...
export default function TaskDetailRoute() {
  ...
  const selectedAssigneeName =
    users.find((member) => member.id === assignedToId)?.name ?? task?.assignedTo.name ?? "Select assignee";
  ...
  return (
    <section className="space-y-6">
      <Card className="overflow-hidden border-border/80">
        ...
        <CardHeader className="space-y-4">
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusClasses[task.status]}`}>
                {task.status}
              </span>
              <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${priorityClasses[task.priority]}`}>
                {task.priority}
              </span>
            </div>
            <div className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2 lg:grid-cols-4">
              <p>
                Assigned: <span className="font-medium text-foreground">{task.assignedTo.name}</span>
              </p>
              <p>
              ...
              <p>
                Last activity: <span className="font-medium text-foreground">{toRelativeTime(task.lastActivityAt)}</span>
              </p>
            </div>
          </div>
        </CardHeader>
        <CardContent className="grid gap-4 border-t bg-muted/20 py-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={changeStatusMutation.isPending || !taskStatus || taskStatus === task.status}
              >
          ...
          <div className="space-y-2">
            ...
            {!canManageTask ? <p className="text-xs text-muted-foreground">Only admins/managers can reassign.</p> : null}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Keep updates concise and structured. Use the <span className="font-medium text-foreground">Submit Update</span>{" "}
        ...
        <CardContent>
          ...
          {timelineQuery.isError ? (
            <div className="rounded-md border border-dashed p-4">
              <p className="text-sm text-muted-foreground">{getApiErrorMessage(timelineQuery.error)}</p>
            </div>
          ) : null}
          ...
          {timelineQuery.isSuccess && timelineItems.length > 0 ? (
            <div className="space-y-4">
              {timelineItems.map((item, index) => (
                <article key={`${item.createdAt}-${index}`} className="relative pl-8">
                  ...
                  {item.type === "update" ? (
                    <div className="rounded-lg border bg-card p-4 shadow-sm">
                      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                        <p className="text-sm font-medium">
                          {item.update.user.name} <span className="text-muted-foreground">({item.update.user.role})</span>
                        </p>
                        <p className="text-xs text-muted-foreground">{formatDate(item.createdAt)}</p>
                      </div>
                      <div className="space-y-2">
                        <div>
                          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">What was done</p>
                    ...
                    <div className="rounded-lg border border-dashed bg-muted/25 p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <p className="text-sm">
                          <span className="font-medium">{timelineEventLabel[item.log.actionType] ?? item.log.actionType}</span>{" "}
                          by {item.log.user.name} ({item.log.user.role})
                        </p>
                        <p className="text-xs text-muted-foreground">{formatDate(item.createdAt)}</p>
                      </div>
                    </div>
