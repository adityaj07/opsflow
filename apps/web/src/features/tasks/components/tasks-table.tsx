...
export function TasksTable({
  ...
  const columns = [
    ...
    columnHelper.accessor('status', {
      ...
      cell: info => (
        <span
          className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusClasses[info.getValue()]}`}
        >
          {info.getValue()}
        </span>
      ),
    ...
    columnHelper.accessor('assignedToId', {
      ...
      cell: info => {
        ...
        const name = info.row.original.assignedToName ?? usersById.get(assignedToId) ?? 'Unknown';
    ...
    columnHelper.accessor('createdById', {
      ...
      cell: info => {
        ...
        const name = info.row.original.createdByName ?? usersById.get(createdById) ?? 'Unknown';
    ...
    columnHelper.accessor('priority', {
      ...
      cell: info => (
        <span
          className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${priorityClasses[info.getValue()]}`}
        >
          {info.getValue()}
        </span>
      ),
